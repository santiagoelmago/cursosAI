...
<div class="mb-8 flex items-center justify-between">
  <div>
    <h1 class="text-3xl font-bold mb-1">{$t('dashboard.welcomePrefix')}{user?.name?.split(' ')[0] || 'Creator'}</h1>
    <p class="text-surface-500 dark:text-surface-400">{$t('dashboard.subheading')}</p>
  </div>
  <a href="/courses/new">
    <Button>
...
<div class="grid sm:grid-cols-3 gap-4 mb-8">
  <div class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-xl p-5">
    <div class="flex items-center gap-3 mb-2">
      ...
      <span class="text-sm text-surface-500">{$t('dashboard.totalCourses')}</span>
    </div>
  ...
  <div class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-xl p-5">
    <div class="flex items-center gap-3 mb-2">
      ...
      <span class="text-sm text-surface-500">{$t('dashboard.published')}</span>
    </div>
  ...
  <div class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-xl p-5">
    <div class="flex items-center gap-3 mb-2">
      ...
      <span class="text-sm text-surface-500">{$t('dashboard.students')}</span>
    </div>
...
<div>
  <div class="flex items-center justify-between mb-4">
    <h2 class="text-xl font-bold">{$t('dashboard.yourCourses')}</h2>
    <a href="/courses" class="text-sm font-medium text-brand-600 dark:text-brand-400 hover:underline">{$t('dashboard.viewAll')}</a>
  </div>
  ...
  {#if courses.length === 0}
    <div class="bg-white dark:bg-surface-900 border border-dashed border-surface-300 dark:border-surface-700 rounded-2xl p-12 text-center">
      ...
      <h3 class="font-semibold text-lg mb-2">{$t('dashboard.noCourses')}</h3>
  ...
  {:else}
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <a href="/courses/{course.id}" class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-xl p-5 hover:border-brand-500 dark:hover:border-brand-500 transition-colors group block">
          <div class="flex items-start justify-between mb-3">
            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {course.status === 'published' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-surface-100 text-surface-600 dark:bg-surface-800 dark:text-surface-400'}">
              {course.status === 'published' ? $t('dashboard.published') : $t('dashboard.draft')}
            </span>
          ...
          <p class="text-sm text-surface-500 dark:text-surface-400 line-clamp-2">{course.description || $t('dashboard.noDescription')}</p>
      ...
      <a href="/courses/new" class="bg-white dark:bg-surface-900 border border-dashed border-surface-300 dark:border-surface-700 rounded-xl p-5 flex flex-col items-center justify-center text-center hover:border-brand-400 dark:hover:border-brand-500 transition-colors group">
        ...
        <p class="text-sm font-medium text-surface-500 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{$t('dashboard.newCourse')}</p>
      </a>
    </div>
  {/if}
</div>
